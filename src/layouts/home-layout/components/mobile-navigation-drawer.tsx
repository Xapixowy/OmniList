...
const MobileNavigationItem = ({ title, link, variant, className }: NavigationItem & { className?: string }) => {
  ...
  const generateClassName = ({ isActive }: NavLinkRenderProps) => {
    ...
    return `${className} ${defaultClasses} ${isActive ? activeClasses : inactiveClasses}`;
  };
...
const MobileNavigationSection = ({ id, title, items }: NavigationSection) => {
  const [isCollapsed, setIsCollapsed] = useState<boolean>(false);

  return (
    <div className='flex flex-col'>
      <button className='link flex items-center justify-between px-8 py-6' onClick={() => setIsCollapsed(!isCollapsed)}>
